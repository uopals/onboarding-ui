...
const server = setupServer(
  ...
  http.get(
    ...
    (req) => {
      ...
      return HttpResponse.json({ valid: false, message: 'Invalid corporation number' }, { status: 404 });
    }
  )
);
...
describe('Onboarding View', () => {
  ...
  it('shows validation errors for empty fields and invalid input', async () => {
    render(<Onboarding />);
		...
		fireEvent.change(screen.getByLabelText(/Corporation Number/i), { target: { value: 'invalid' } });
		fireEvent.blur(screen.getByLabelText(/Corporation Number/i));

    await waitFor(() => {
      expect(screen.getByText(/firstName is a required field/i)).toBeInTheDocument();
      expect(screen.getByText(/lastName is a required field/i)).toBeInTheDocument();
      expect(screen.getByText(/Phone number must have 10 digits/i)).toBeInTheDocument();
  ...
  it.skip('submits the form successfully with valid data', async () => {
    render(<Onboarding />);
    fireEvent.change(screen.getByLabelText(/Corporation Number/i), { target: { value: validCorporationNumber } });
    fireEvent.change(screen.getByLabelText(/First Name/i), { target: { value: 'Jane' } });
    fireEvent.change(screen.getByLabelText(/Last Name/i), { target: { value: 'Smith' } });
    fireEvent.change(screen.getByLabelText(/Phone Number/i), { target: { value: '4161234567' } });
